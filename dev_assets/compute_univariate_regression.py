#!/usr/bin/env python
"""
Compute univariate (per-attribute) regression between OMI attribute means and face mesh landmarks.

For each attribute, regress each landmark coordinate on that attribute alone.
We use standardized variables so the output is comparable across attributes.

Outputs:
- omi/corr/univariate_regression.npz
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute univariate regression between attributes and landmarks."
    )
    parser.add_argument(
        "--attr_csv",
        type=Path,
...
def main() -> int:
    ...
    for a in range(X.shape[1]):
        ...
        if x_std == 0:
            x_std = 1.0
        xz = (x - x_mean) / x_std
        coef[a] = (xz[:, None] * Yz).sum(axis=0) / (n_samples - 1)
